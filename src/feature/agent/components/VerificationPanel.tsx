    ...
    <div className="w-full bg-[#0a0f0d] border border-[rgba(0,255,85,0.15)] rounded-2xl p-5 shadow-[0_0_20px_rgba(0,255,85,0.02)] flex flex-col h-[480px]">
      ...
      <div className="grid grid-cols-3 gap-2.5 mb-4 font-mono">
        <div className="bg-slate-950 border border-slate-900 rounded-xl p-3 text-center">
          <span className="text-[9px] uppercase text-slate-500 block">Agent Accuracy</span>
          <span className="text-base font-bold text-[#00ff55]">{avgAccuracy}%</span>
        </div>
        <div className="bg-slate-950 border border-slate-900 rounded-xl p-3 text-center">
          <span className="text-[9px] uppercase text-slate-500 block">Active Reliability</span>
          <span className="text-base font-bold text-blue-400">Class-A</span>
        </div>
        <div className="bg-slate-950 border border-slate-900 rounded-xl p-3 text-center">
          <span className="text-[9px] uppercase text-slate-500 block">Total Verified</span>
      ...
      <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-800">
        {verifications.length === 0 ? (
          <div className="text-slate-600 text-center py-20 italic font-mono text-xs leading-relaxed">
            ...
            <span className="text-[10px] mt-1 block">Verification automatically compares Cycle N-1 predictions with Cycle N actual data.</span>
          </div>
        ) : (
          <div className="space-y-3">
            {verifications.map((item, idx) => (
              <div 
                key={idx}
                className="bg-slate-950 border border-slate-900 rounded-xl p-3 font-mono text-[10px]"
              >
                <div className="flex justify-between items-center mb-2">
                  <span className="text-slate-300 font-semibold">{item.zoneName}</span>
                  <span className={`text-[8px] px-2 py-0.5 rounded font-bold uppercase tracking-wider ${getStatusColor(item.status)}`}>
                    {item.status} ({item.accuracy}%)
                  </span>
                </div>

                <div className="grid grid-cols-2 gap-1 border-t border-slate-900/50 pt-2 text-slate-500 text-[9px]">
                  <div>
                    Predicted Delta: 
